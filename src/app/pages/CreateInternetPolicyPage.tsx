...
type SourceRow = {
  id: string;
  type: 'Users' | 'Groups';
  selectedItems: string[];  // checked/selected — shown as chips in trigger
  addedGroups?: string[];   // all groups added to the dropdown list
};
...
export function CreateInternetPolicyPage() {
  ...
  // Group search state (per source row)
  const [groupSearch, setGroupSearch] = useState<Record<string, string>>({});

  // Manual group name input (per source row, Day 0)
  const [groupInput, setGroupInput] = useState<Record<string, string>>({});

  // CSV upload modal state
  type CsvFile = { name: string; rowCount: number };
  const [csvFiles, setCsvFiles] = useState<Record<string, CsvFile[]>>({});
  const [csvModalRowId, setCsvModalRowId] = useState<string | null>(null);
  // null = idle, 'uploading' = progress, 'done' = success
              ...
              <div className="flex-1 flex flex-col gap-[8px]">
                ...
                {row.type === 'Groups' ? (
                  ...
                  <div className="flex flex-col gap-[8px]">
                          ...
                          {(row.addedGroups ?? []).length === 0 ? (
                            <div className="flex flex-col items-center gap-[10px] pt-[28px] pb-[20px] px-[20px] text-center">
                              ...
                              <div>
                                ...
                                <p className="text-[11px] text-[#9ca3af] mt-[8px] leading-[16px] flex items-start gap-[5px]">
                            ...
                            /* Groups list — click row or checkbox to select */
                    ...
                    {(csvFiles[row.id]?.length ?? 0) > 0 && (
                      <div className="flex flex-col gap-[4px]">
                        {csvFiles[row.id]!.map((f, i) => (
                          <div key={i} className="flex items-center justify-between bg-[#f9fafb] border border-[#e5e7eb] rounded-[6px] px-[10px] py-[6px]">
                            <div className="flex items-center gap-[8px] min-w-0">
                              <FileSpreadsheet className="w-[13px] h-[13px] text-[#0b8aeb] shrink-0" />
                              <span className="text-[12px] font-medium text-[#101828] truncate">{f.name}</span>
                              <span className="text-[11px] text-[#9ca3af] shrink-0">{f.rowCount} entries</span>
                            </div>
                            <button
                              onClick={() => setCsvFiles((prev) => ({ ...prev, [row.id]: prev[row.id]!.filter((_, idx) => idx !== i) }))}
                              className="text-[#9ca3af] hover:text-[#d4183d] shrink-0 ml-[8px]"
                            >
                  ...
                  <div className="relative">
                      ...
                      <div className="flex flex-wrap gap-[5px]">
                        {row.selectedItems.map((item) => (
                          ...
                          >
                            ...
                            <button onClick={() => removeSourceItem(row.id, item)} className="text-[#9ca3af] hover:text-[#6a7282]">
                              <X className="w-3 h-3" />
                            </button>
                          </div>
                        ))}
                    ...
                    {openDropdowns[`source-items-${row.id}`] && (
                      <div data-dropdown-menu className="absolute top-full mt-1 left-0 right-0 bg-white border border-[#e5e7eb] rounded-[8px] shadow-lg z-50">
                        ...
                        <div className="max-h-[160px] overflow-y-auto">
                          {USERS_LIST.filter((u) => u.toLowerCase().includes((groupSearch[row.id] ?? '').toLowerCase())).map((item) => (
                            <div
                              key={item}
                              onClick={() => toggleSourceItem(row.id, item)}
      ...
      {csvModalRowId !== null && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center">
          ...
          <div
            ...
            onClick={() => {
              if (csvModalStage !== 'uploading') setCsvModalRowId(null);
            }}
          />
                  ...
                  <div className="flex items-center justify-between bg-[#f9fafb] rounded-[8px] px-[14px] py-[10px]">
                    <div className="flex items-start gap-[10px] min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[12px] text-[#6a7282] mt-[1px]">
                          {csvModalRowCount} entries · Group data provisioned
                        </p>
                      </div>
                    </div>
                    <button
                  ...
                  <button
                    onClick={() => {
                      setCsvFiles((prev) => ({
                        ...
                        [csvModalRowId!]: [
                          ...(prev[csvModalRowId!] ?? []),
                          { name: csvModalFileName, rowCount: csvModalRowCount },
                        ],
                      }));
